...
exports.editModule = async (req, res, next) => {
    ...
    try {
        ...
        if (req.body.name) {
            ...
            await Module.updateOne({ _id: id }, { $set: { name: req.body.name } });
        }
        if (req.body.description) {
            await Module.updateOne({ _id: id }, { $set: { description: req.body.description } });

        }
        if (req.body.fee) {
            await Module.updateOne({ _id: id }, { $set: { fee: req.body.fee } });

        }
        if (req.file) {
            await Module.updateOne({ _id: id }, { $set: { image: req.file.path } });
            console.log(req.file)
        }
        const updatedModule = await Module.findOne({ _id: id });
